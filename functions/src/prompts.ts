/**
 * Server-side system prompts + model configuration (NOT client-controlled).
 * Extracted from index.ts (WS12-03) so prompt text and model config can be
 * reviewed/diffed independently of request-handling logic. Side-effect-free:
 * no imports that run code, no disk reads, no admin.* calls.
 */
...
export const MODEL_CONFIG: Record<string, { model: string; max_tokens: number; temperature?: number }> = {
...
export const MINOR_SAFETY_PROMPT = `MINOR USER SAFEGUARDS (this user is under 18):
